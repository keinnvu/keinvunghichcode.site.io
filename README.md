...
<body>
    <div class="falling-photos-container">
    <script>
    document.addEventListener("DOMContentLoaded", () => {
    const container = document.querySelector(".falling-photos-container");

    const imageSources = [
        "image/giftbox.png", // Thay bằng đường dẫn ảnh của bạn
        ];
    ...
    function createFallingPhoto() {
        ...
        photo.src = imageSources[randomIndex];
        
        // Cài đặt vị trí ban đầu ngẫu nhiên
        photo.style.left = `${Math.random() * 100}vw`;

        // Cài đặt kích thước ngẫu nhiên
        const size = Math.random() * 50 + 50; // Kích thước từ 50 đến 100px
        photo.style.width = `${size}px`;
        photo.style.height = 'auto';

        // Cài đặt thời gian và độ trễ hoạt ảnh ngẫu nhiên
        const duration = Math.random() * 10 + 5; // Thời gian rơi từ 5 đến 15s
        photo.style.animationDuration = `${duration}s`;
        photo.style.animationDelay = `-${Math.random() * duration}s`; // Đặt độ trễ âm để tạo sự liên tục

        container.appendChild(photo);

        // Xóa ảnh khỏi DOM sau khi rơi xong để tiết kiệm tài nguyên
        photo.addEventListener("animationend", () => {
            photo.remove();
        });
    }

    // Tạo các ảnh rơi ban đầu
    for (let i = 0; i < 20; i++) {
        createFallingPhoto();
    }

    // Tiếp tục tạo ảnh rơi sau mỗi khoảng thời gian nhất định
    setInterval(createFallingPhoto, 1000); // Mỗi giây tạo thêm một ảnh
});
</script>    
    </div>
<style>
body {
    margin: 0;
    overflow: hidden; /* Ẩn các hình ảnh khi chúng rơi ra khỏi màn hình */
    background-color: #f0f0f0;
}

.falling-photos-container {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    pointer-events: none; /* Cho phép click xuyên qua lớp chứa ảnh rơi */
}

.falling-photo {
    position: absolute;
    animation-name: fall; /* Tên hoạt ảnh được định nghĩa bên dưới */
    animation-timing-function: linear; /* Hoạt ảnh di chuyển đều */
    animation-iteration-count: infinite; /* Lặp lại vô hạn */
    opacity: 0; /* Ban đầu ẩn các hình ảnh */
}

/* Định nghĩa hoạt ảnh "fall" */
@keyframes fall {
    0% {
    ...
    10% {
        opacity: 1; /* Hiển thị ảnh sau khi bắt đầu rơi một chút */
    }
    100% {
        transform: translateY(100vh) rotate(360deg);
        opacity: 0; /* Ẩn ảnh khi rơi đến cuối màn hình */
    }
}
    
</style>
  
    <audio id="bg-music" src="minh.mp3" loop></audio>
    ...
    <div id="castle" style="display: none;">
        <div class="letter">
            <div class="valentines">
                ...
                <div class="front"></div>
                <div class="card">
                    <div class="text">Send to</br>beautiful girl on</br>20/10</div>
                    <div class="heart"></div>
                </div>
                <div class="hearts">
                    <div class="one"></div>
...
html, body {
  ...
  margin: 0;
  padding: 0;
  background-color: #fcb3e6; /* xanh biển nhạt */
}
...
@keyframes pulse {
  ...
  70% {
    ...
    box-shadow: 0 0 20px 10px rgba(205, 92, 92, 0);
  }
  100% {
    transform: translateX(-50%) scale(1);
...
(() => {
    ...
    // Khi ấn nút thì chuyển cảnh
    document.querySelector("#text-click").addEventListener("click", function () {
    ...
    <img src="./image/mewmew.gif" class="corner-gif top-left" alt="">
<img src="./image/mewmew.gif" class="corner-gif top-right" alt="">
<img src="./image/mewmew.gif" class="corner-gif bottom-left" alt="">
<img src="./image/mewmew.gif" class="corner-gif bottom-right" alt="">
<style>
    /* 🪄 GIF 4 góc */
.corner-gif {
  position: fixed;   /* giữ cố định khi cuộn */
  width: 120px;      /* kích thước bạn có thể chỉnh */
  z-index: 1;
}
...
gifs.forEach(img => {
  ...
  setInterval(() => {
    img.src = '';
    img.src = originalSrc + '?t=' + new Date().getTime(); // trick tránh cache
  }, 5000); // 5000ms = 5 giây, chỉnh theo thời lượng GIF của bạn
});
</script>
    ...
    <script>
        ...
        const textLetterH2 = "Nhân dịp 20/10 !";
        ...
        const textLetterP = "Chúc em ngày hôm nay và mai sau luôn rạng rỡ như những đóa hoa, mạnh mẽ theo cách riêng của mình và hạnh phúc với những gì đang có. Mong rằng cuộc sống của em luôn ngập tràn yêu thương, tiếng cười và những điều tốt đẹp nhất 💗💗💗💗💗💗💗💗💗💗💗💗 ";
        ...
        var lineDrawing = anime({
            ...
            complete: function (anim) {
                //Leaf One
                leafOne.setAttribute("fill", "#33CC00");
                leafOne.setAttribute("stroke", "none");
                //Leaf Two 
                leafTwo.setAttribute("fill", "#33CC00");
                leafTwo.setAttribute("stroke", "none");
                //Stick
                stickLine.setAttribute("fill", "#66FF33");
                stickLine.setAttribute("stroke", "none");
                // Leaf S1
                leafS1.setAttribute("fill", "#33CC00");
                leafS1.setAttribute("stroke", "none");
                // Rose 1
                rose1.setAttribute("fill", "#EE0000");
                rose1.setAttribute("stroke", "none");
                // Rose 2
                rose2.setAttribute("fill", "#CC0000");
                rose2.setAttribute("stroke", "none");
                // Rose 3
                rose3.setAttribute("fill", "#EE0000");
                rose3.setAttribute("stroke", "none");
                // Rose 3
                rose4.setAttribute("fill", "#CC0000");
